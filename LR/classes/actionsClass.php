<?php 
    ...
    class ActionBrand {
        ...
        public static function addAction(string $name_) : bool{
            ...
            $query = "INSERT INTO brand (`id_brand`, `name_brand`) VALUES ( NULL, '$name')";
        ...
        public static function editAction(int $id_, string $name_) : bool{
            ...
            $name = mysqli_real_escape_string($mysql,$name_);
            $query = "UPDATE brand SET  name_brand = '$name' WHERE id_brand = $id";
            $result = $mysql->query($query);
            return $result;
        }
        ...
        public static function deleteAction(int $id_) : bool{
            $mysql = Database::connection();
            $id = mysqli_real_escape_string($mysql,$id_);
            $query = "DELETE FROM brand WHERE id_brand = $id";
            $result = $mysql->query($query);
            return $result;
        }
        ...
        public static function showAction() : array{
            $mysql = Database::connection();
            $query = "SELECT id_brand, name_brand  FROM brand ORDER BY id_brand";
            $result = $mysql->query($query);
            $arr = [];
        ...
        public static function getByIdAction(int $id_) : array{
            $mysql = Database::connection();
            $id = mysqli_real_escape_string($mysql,$id_);
            $query = "SELECT id_brand, name_brand  FROM brand WHERE id_brand = $id";
            $result = $mysql->query($query);
            $arr = [];
            while ($row = $result->fetch_assoc()) {
                $arr[] = $row;
    ...
    class ActionGoods {
        ...
        public static function showAction(int $id) : array{
            $mysql = Database::connection();
            if ($id == 0){
                $query = "SELECT goods.id, goods.name,  goods.description, goods.cost, brand.name_brand, goods.img_path FROM goods INNER JOIN brand ON brand.id_brand = goods.id_brand ORDER BY id";
            }else{
                $query = "SELECT goods.id, goods.name,  goods.description, goods.cost, brand.name_brand, goods.img_path FROM goods INNER JOIN brand ON brand.id_brand = goods.id_brand WHERE goods.id_brand = $id";
            }
